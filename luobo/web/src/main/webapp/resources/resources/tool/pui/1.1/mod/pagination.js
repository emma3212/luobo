...
*   ***********************************************************************************************
*   9、根据后台返回数据pageSize, 计算分页信息, 如果, 返回数据中存在pageSize， 则按照pageSize计算
*       否则，按照配置项中pageSize计算
*       by wangjing (2014-8-26)
*   10、将 afterDataLoad从判断中提到数据加载之后执行。
*       by wangjing (2015-2-6) 
*   11、修复组件支持一页，多个分页组件的bug: (by wangjign 2015-3-13)
*       (1) 配置项使用clone方法复制了一套config配置，防止多次调用时，参数覆盖
*       (2) 事件绑定使用事件代理，使多个分页组件的事件不互相影响
*   12、修复调用reload方法后，页码信息参数不变的问题，将merge改为mix，不重新创建配置参数 (by wangjign 2015-3-14)
* ---------------------------------------------------------------------------*/
...
KISSY.add('pagination/oper', function(S, Pctrl, Juicer){
    ...
    S.augment(oper, S.EventTarget, {
        ...
        _renderData: function(data){
            var
                that = this, opts = that.opts, dataList = [], len, i, pSize,
            ...
            if(S.isEmptyObject(data) || !data || data.dataCount == 0) {
                ...
                /**
                 * 如果后台返回数据中存在pageSize，则按照后台设定分页数量计算
                 * 否则按照组件配置计算
                 * by wangjing
                 * 2014-8-26
                 */
                ...
                opts.pageCount = Math.ceil(opts.dataCount/ pSize);
                ...
                if(len == 0) {
                    ...
                    if(!(S.isEmptyObject(that.prevPaginationData))){
                        //多传递了pageSize参数
                        that.pctrl.refresh(opts.page, opts.dataCount, pSize);    
                    }
            ...
            //by wangjing 2015.2.6 将 afterDataLoad从判断中提到数据加载之后执行。
...
KISSY.add('pagination/pctrl', function(S){
    ...
    S.augment(pctrl, S.EventTarget, {
         ...
         */
        refresh: function(page, dataCount, pageSize){
            var
                that = this, opts = that.opts, pSize;
            /**
             * 如果pageSize存在则按照后台返回的pageSize计算
             * by wangjing
             * 2014-8-26
             */
            if(pageSize){
