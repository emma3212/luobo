...
KISSY.add('defender/core',function(S,Pattern,Shell,Tip,FormErrorTip,FloatTip, InsideTip, InlineTip){
	var
		...
		//输入框中无提示图表，当提示图表在输入框中是生效 by wangjing
		FORM_NO_ICON_CLASS = 'no-icon',
		//时间询问间隔时间
		TIMER_INTERVAL      = .2,
		CONFIG 				= {
			showEvent:'focus',
			...
			zIndex:101,
			theme:'inline',
			//显示输入框中的图表，当提示布标在输入框中显示时才生效 by wangjing 
			showIcon: true,//不显示图表为false
			width: 400,
			direction: 'right',
			alignY:'center',
			alignX:'center',
	...
	S.augment(Validation,S.EventTarget,{
		...
		_renderFormStyle: function(){
			...
			/**
			 * 当showIcon为FALSE时，提示图表不在输入框内部显示
			 * 适用场合：提示信息在输入框中显示，在form上添加class="no-icon"
			 * by wangjing
			 */
			if(!opts.showIcon){
				$(f).addClass(FORM_NO_ICON_CLASS);
			}
		},
		/**
		 * 初始化每个域
...
KISSY.add('validation/inline-tip', function(S){
	...
	S.augment(InlineTip, {
		init: function(){
			...
			//备份错误信息
			that.reserveErrTip = $(that.field).data('tip').split('|');
			//错误提示
