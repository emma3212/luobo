...
* 	初始生成 
* ---------------------------------------------------------------------------*/
KISSY.add('page/product/brand', function (S, Core) {
    PW.namespace('page.product.Brand');
    PW.page.product.Brand = function(param){
        new Core(param);
    }
},{
    requires:[
...
KISSY.add('brand/core', function(S){
	var
		...
		DATA_TYPE = 'data-type',
		SUCCESS_TIP = ['新增品牌成功！', '修改品牌成功！', '状态修改成功！'],
	...
	S.augment(Core, {
		...
		_newSave: function(tr){
			...
			ProductIO.newBrand(extraData, function(code, errMsg){
				if(code){
		...
		_editSave: function(tr){
			var
				...
				statusStr = S.trim(DOM.html(query('td', tr)[1])),
				data = S.merge(DOM.serialize(el.form), {id: id});
			ProductIO.editBrand(data, function(code, errMsg){
				if(code){
		...
		_enlightHandler: function(e){
			var
				...
				tr = DOM.parent(e, 'tr'),
				statusStrEl = query('td', tr)[1],
				id = DOM.attr(tr, DATA_ID);
			Dialog.confirm(CONFIRM_TIP, function(){
				ProductIO.enlightBrand({id: id}, function(code, errMsg){
					if(code){
						switch(statusTip){
							case ENLIGHT_TIP[0]:
		...
		_getRenderData: function(trObj, type){
			...
			data = {
				id: id,
				brandName: datas[0],
				statusStr: (type == 'edit') ? datas[1] : '已'+ENLIGHT_TIP[1],
				type: type
			};
			return data;
		},
