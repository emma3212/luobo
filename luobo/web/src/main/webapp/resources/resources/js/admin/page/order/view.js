...
* 	初始生成 
* ---------------------------------------------------------------------------*/
KISSY.add('page/order/view', function (S, Core) {
    PW.namespace('page.order');
    PW.page.order.View = function(){
    	return new Core();
    }
},{
    requires:[
	...
	S.augment(Core, {
		...
		_saveHandler: function(e){
			...
			that.defenderObj.validAll(function(rs){
				OrderIO.saveReturn(data, function(code, errMsg){
					if(code){
		...
		_saveTip: function(target, tipHolderEl){
			...
			OrderIO.viewTip({id:id, comment:newContent},function(code, errMsg){
                if(code){
		...
		_repayTogetherHanlder: function(){
			...
			Dialog.confirm(ONEPRESS_REPAY_TIP, function(){
				OrderIO.repayPress({id: id}, function(code, errMsg){
					if(code){
